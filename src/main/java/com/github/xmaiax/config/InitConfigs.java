...
public class InitConfigs {
  ...
  private static final boolean JAVA_UI_RESIZABLE = false;
  private static final boolean JAVA_UI_ALWAYS_ON_TOP = true;
  ...
  private static final boolean JAVA_UI_FLAG_FULLSCREEN_DEFAULT = true;
  private static final boolean JAVA_UI_FLAG_VSYNC_DEFAULT = true;
  private static final boolean JAVA_UI_FLAG_DEBUG_MODE_DEFAULT = true;
  ...
  private static String getValuePropertyNameFromVideoSettings(String fieldName) {
    return Arrays.asList(VideoSettings.class.getConstructors()[0].getParameters())
  ...
  private static class Resolution {
    public Resolution(java.awt.DisplayMode displayMode) {
    ...
    @Override public boolean equals(Object that) {
      if(!(that instanceof Resolution)) return false;
  ...
  public static void initConfig(org.springframework.boot.SpringApplication springApp, String... args) throws Exception {
    final Map<String, String> applicationProperties = Arrays.asList(java.nio.charset.StandardCharsets.UTF_8.decode(
      com.github.xmaiax.App.getBufferFromResource("application.properties")).toString().split("\n")).stream()
          .filter(str -> str != null && !str.isBlank()).collect(
        Collectors.toMap(str -> str.toString().split(PROPERTIES_SEPARATOR)[0],
                         str -> str.toString().split(PROPERTIES_SEPARATOR)[1]));
    final String title = applicationProperties.get(getValuePropertyNameFromVideoSettings(PROP_PREFIX_TITLE)).toString();
    ...
    final java.util.List<Resolution> allResolutions = Arrays.asList(java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment()
      .getScreenDevices()).stream().map(screenDevices -> Arrays.asList(screenDevices.getDisplayModes()))
        .flatMap(java.util.Collection::stream).map(displayMode -> new Resolution(displayMode)).collect(Collectors.toSet())
          .stream().sorted((x, y) -> x.getWidth().compareTo(y.getWidth())).collect(Collectors.toList());
    ...
    final JCheckBox fullscreenCheckbox = new JCheckBox(JAVA_UI_FULLSCREEN_LABEL, JAVA_UI_FLAG_FULLSCREEN_DEFAULT);
    fullscreenPanel.add(fullscreenCheckbox);
    panel.add(fullscreenPanel);
    final JPanel vsyncPanel = new JPanel();
    final JCheckBox vsyncCheckbox = new JCheckBox(JAVA_UI_VSYNC_LABEL, JAVA_UI_FLAG_VSYNC_DEFAULT);
    vsyncCheckbox.setEnabled(true);
    vsyncPanel.add(vsyncCheckbox);
    panel.add(vsyncPanel);
    final JPanel debugModePanel = new JPanel();
    final JCheckBox debugModeCheckbox = new JCheckBox(JAVA_UI_DEBUG_MODE_LABEL, JAVA_UI_FLAG_DEBUG_MODE_DEFAULT);
    ...
    startButton.addActionListener((e) -> {
      ...
      Resolution selectedResolution = (Resolution) allResolutions.toArray()[resolutionsComboBox.getSelectedIndex()];
      props.setProperty(getValuePropertyNameFromVideoSettings(PROP_PREFIX_RESOLUTION_WIDTH), selectedResolution.getWidth().toString());
      props.setProperty(getValuePropertyNameFromVideoSettings(PROP_PREFIX_RESOLUTION_HEIGHT), selectedResolution.getHeight().toString());
      props.setProperty(getValuePropertyNameFromVideoSettings(PROP_PREFIX_FULLSCREEN), Boolean.toString(fullscreenCheckbox.isSelected()));
      props.setProperty(getValuePropertyNameFromVideoSettings(PROP_PREFIX_VSYNC), Boolean.toString(vsyncCheckbox.isSelected()));
      props.setProperty("logging.file", String.format("./%s.log", applicationProperties.get("app.info.release-name")));
      ...
      props.setProperty(String.format("logging.level.%s", applicationProperties.get("app.info.release-package")),
        debugModeCheckbox.isSelected() ? "DEBUG" : "WARN");
      springApp.setDefaultProperties(props);
      springApp.run(args);
    });
    final ImageIcon appIcon = getImageIconFromProperty(applicationProperties, PROP_WINDOW_ICON_LOCATION);
    ...
    aboutButton.addActionListener((e) -> {
      javax.swing.JOptionPane.showMessageDialog(configFrame, String.format(
        ...
        applicationProperties.get("app.info.contact.email")), "About ".concat(title),
          javax.swing.JOptionPane.QUESTION_MESSAGE, getImageIconFromProperty(applicationProperties, PROP_ABOUT_IMAGE_LOCATION));
    });
