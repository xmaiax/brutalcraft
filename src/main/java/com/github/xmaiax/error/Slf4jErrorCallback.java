...
public class Slf4jErrorCallback extends org.lwjgl.glfw.GLFWErrorCallback {
  ...
  public void invoke(int error, long description) {
    ...
    java.util.stream.IntStream.range(0, Thread.currentThread().getStackTrace().length).forEach(index -> {
      if(index >= LWJGL_STACK_HEIGHT) LOGGER.error(Thread.currentThread().getStackTrace()[index].toString());
    });
  }
